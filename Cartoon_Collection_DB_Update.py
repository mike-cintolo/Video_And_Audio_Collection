...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 22 12:56:01 2023
...
valid_extensions = ['wmv', 'mp4', 'avi', 'mkv', 'ogv', 'webm', 'mpg', 'mov']
...
def parse_file_name(filename):
    ...
    show_title, production_year, season_number, episode_number, episode_title, file_ext, is_movie = "Unknown", "1900", "0", "0", "Unknown", "Unknown", "0"
    ...
    if season_number == "0" and episode_number == "0" and episode_title == "Unknown":
        is_movie = "1"
        episode_title = "Movie"
    ...
    return show_title, production_year, season_number, episode_number, episode_title, file_ext, is_movie
...
def insert_file_data(file_path):
    ...
    try:
        ...
        # Check if the file name exists in the database for CartoonCollection table
        query_collection = "SELECT COUNT(*) FROM Entertainment.CartoonCollection WHERE File_Name = %s"
        cursor.execute(query_collection, (file_name,))
        result_collection = cursor.fetchone()
        if result_collection[0] > 0:
            print(f"File name already exists in the CartoonCollection database: {file_name}")
            return  # Skip the insert if the file name exists in CartoonCollection

        # Parse the file name
        show_title, production_year, season_number, episode_number, episode_title, file_ext, is_movie = parse_file_name(file_name)

        # Example SQL query to insert data into the CartoonCollection table
        insert_query_collection = "INSERT INTO Entertainment.CartoonCollection (Show_Title, Production_Year, Season_Number, Episode_Number, Episode_Title, Is_Movie, File_Ext, File_Name) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
        values_collection = (show_title, production_year, season_number, episode_number, episode_title, is_movie, file_ext, file_name)

        cursor.execute(insert_query_collection, values_collection)
        db.commit()

        # Log the successfully inserted file
        log_successful_insert(file_name, show_title, production_year, is_movie)
...
def log_successful_insert(file_name, show_title, production_year, is_movie):
    success_data = {
    ...
    success_file = '/home/michaelcintolo/Programming/OutputFiles/Cartoon_Collection_Successful_Inserts.csv'
...
def log_error(file_path, error_message):
    error_log_file = '/home/michaelcintolo/Programming/OutputFiles/Cartoon_Collection_Error_Log.txt'
